...
def checkFile(path):
    try:
        ...
        file.close()
        # 写入resultDict
        resultDict = {'systemName': systemName, 'dataDate': dataDate, 'fileName': fileName,
                      'fileSize': fileSize, 'fileNum': fileNum, 'fileEncod': fileEncod}
        print resultDict
...
def explore(dir):
    for root, dirs, files in os.walk(dir):
        for file in files:
            ...
            resultFile = "E:\\dsa/checkFileResult" + \
                dataDate.replace("-", "")+".check"
            ...
            file.write(str(resultDict['systemName'])+"~@~"+str(resultDict['dataDate'])+"~@~"+str(resultDict['fileName'])+"~@~"+str(resultDict['fileSize'])+"~@~"+str(
                resultDict['fileNum'])+"~@~"+str(resultDict['fileEncod'])+"~@~"+str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))+"\n")
            file.close()
...
def main():
    ...
    if os.path.isdir(checkPath):
        ...
        print str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
                  )+str("[ERROR]The directory is not exists:%s" % (checkPath))
        sys.exit(1)

    print str(time.strftime('%Y-%m-%d %H:%M:%S',
                            time.localtime(time.time())))+str("[INFO]Check is over")
    sys.exit(0)
